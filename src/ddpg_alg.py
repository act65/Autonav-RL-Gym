...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        action = self.fa3(x)
        action = torch.sigmoid(action) * self.action_limits
...
class DDPG:
    def __init__(self, state_dim, action_dim, ram, action_limits=1.0):
        ...
        self.actor = Actor(self.state_dim, self.action_dim, torch.ones(action_dim) *  self.action_limits)
        self.target_actor = Actor(self.state_dim, self.action_dim, torch.ones(action_dim) * self.action_limits)
    ...
    def get_exploration_action(self, state, var_v=1.0):
        state = torch.from_numpy(state)
        action = self.actor.forward(state).detach()

        noise = np.random.normal(action, var_v, action.shape)
        new_action = action.data.numpy() + noise
        new_action = np.clip(new_action, 0, self.action_limits)
        return new_action

    def optimizer(self):
        s_sample, a_sample, r_sample, new_s_sample = self.ram.sample(BATCH_SIZE)
...
class DDPG_agent:
    ...
    def __init__(self, load_ep, env, max_timesteps):
        ...
        self.trainer = DDPG(STATE_DIMENSION, ACTION_DIMENSION, self.ram, ACTION_V_MAX)
    ...
    def step(self, state, ep):
        ...
        action = self.trainer.get_exploration_action(state, self.var_v)
